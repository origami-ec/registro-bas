 ...
 */
@Stateless(name = "bpmBaseEngine")
public class BpmBaseEngineEJb implements BpmBaseEngine {

    @EJB(beanName = "bpmProcessEngine")
    ...
    @Override
    public Integer getProcessDefinitionVersion(String processDefinitionId) {
        return getProcessEngine().getRepositoryService().createProcessDefinitionQuery().processDefinitionId(processDefinitionId).singleResult().getVersion();
    }
    ...
    public List<Task> getUsertasksList(String asignee, String keyTarea) {
        ...
        try {
            if (keyTarea != null) {
                ...
                acttask = (ArrayList<Task>) taskService.createTaskQuery().taskAssignee(asignee).orderByTaskPriority().desc().orderByExecutionId().asc().list();
            }
        } catch (Exception e) {
    ...
    public List<ProcessDefinition> getAllProcessDesployedList() {
        ...
        return getProcessEngine().getRepositoryService().createProcessDefinitionQuery().list();
    }
...
//            flag = taskQuery != null;
    ...
    public boolean setCandidateUser(String taskId, String candidate) {
        boolean flag;
        try {
            this.getProcessEngine().getTaskService().addCandidateUser(taskId, candidate);
            flag = true;
        } catch (Exception e) {
            flag = false;
            Logger.getLogger(BpmBaseEngine.class.getName()).log(Level.SEVERE, null, e);
    ...
    public boolean deleteCandidateUser(String taskId, String candidate) {
        boolean flag;
        try {
            this.getProcessEngine().getTaskService().deleteCandidateUser(taskId, candidate);
            flag = true;
        } catch (Exception e) {
            flag = false;
            Logger.getLogger(BpmBaseEngine.class.getName()).log(Level.SEVERE, null, e);
    ...
    public Long getCountProcessInstanceHistoric(boolean state) {
        try {
            if (state) {
                return this.getProcessEngine().getHistoryService().createHistoricProcessInstanceQuery().finished().count();
            } else {
                return this.getProcessEngine().getHistoryService().createHistoricProcessInstanceQuery().unfinished().count();
            }
        } catch (Exception e) {
            Logger.getLogger(BpmBaseEngine.class.getName()).log(Level.SEVERE, null, e);
            return 0L;
    ...
    public List<HistoricProcessInstance> getProcessInstanceHistoric(boolean state) {
        ...
        try {
            if (state) {
                hpi = this.getProcessEngine().getHistoryService().createHistoricProcessInstanceQuery().finished().orderByProcessInstanceStartTime().desc().list();
            } else {
                hpi = this.getProcessEngine().getHistoryService().createHistoricProcessInstanceQuery().unfinished().orderByProcessInstanceStartTime().desc().list();
            }
        } catch (Exception e) {
            Logger.getLogger(BpmBaseEngine.class.getName()).log(Level.SEVERE, null, e);
        }
    ...
    public HistoricTaskInstance getLastTaskByProcessInstance(String processInstanceId) {
        try {
            return this.getProcessEngine().getHistoryService().createHistoricTaskInstanceQuery().processInstanceId(processInstanceId).unfinished().singleResult();
        } catch (Exception e) {
            Logger.getLogger(BpmBaseEngine.class.getName()).log(Level.SEVERE, null, e);
        }
        return null;
...
//            this.getProcessEngine().close();
    ...
    public InputStream getProcessDiagram(String definitionID, String diagramName) {
        return getProcessEngine().getRepositoryService().getResourceAsStream(definitionID, diagramName);
    }
    ...
    public InputStream getProcessInstanceDiagram(String procInstId) {
        ...
        System.out.println("getProcessInstanceDiagram: " );
        ...
        if (instance != null) {
            ...
            List<HistoricActivityInstance> list = this.getProcessEngine().getHistoryService().createNativeHistoricActivityInstanceQuery()
                    .sql("SELECT * FROM " + this.getProcessEngine().getManagementService().getTableName(HistoricActivityInstance.class)
                            + " x WHERE x.proc_inst_id_= #{procInstId} ").parameter("procInstId", procInstId).list();
            if (list != null) {
                list.forEach((hai) -> {
