...
/**
 * @author Carlos Loor Vargas Date 28/10/2013
 * @version 2.0 description Valida la cedula en base al digito verificador
 */
public class VerCedulaUtils implements Serializable {
    ...
    public boolean validarCiRuc(char item, String cedula) {
        ...
        switch (item) {
            case 'C':
                ...
                break;
            case 'R':
                if ((cedula.length() == 13) && (cedula.matches("[0-9]*"))) {
                    retorno = true;
                }
                break;
        }
    ...
    public boolean isCIValida(String cedula) {
        System.out.println("// valida cedula " + cedula);
        ...
        try {
            if ((cedula.length() == this.getTamCedula()) && (cedula.matches("[0-9]*"))) {
                ...
                if (provincia > 0 && provincia <= this.getNumProvincias()
                    ...
                    flag = true; //PUSE ESTO AQUI XK NO ENTIENDO QUE DICE =( </3 PERDON CHICOS
    ...
    public boolean isRucValido(String documento) {
        ...
        if ((documento.length() == 13)) {
            ...
            int verificador = this.getVerificador(tercerDigito, documento);
            coef = this.coeficientes(tercerDigito);
            if (coef != null) {
                //suma = this.getvalorRUC(documento, coef);
                suma = this.getSumaRUC(documento, coef);
                //flag = this.getValides(suma, tercerDigito, verificador);
                flag = this.validacionRuc(suma, verificador);
    ...
    public int getVerificador(int digito, String documento) {
        switch (digito) {
            case 6:
                return Integer.parseInt(documento.substring(8, 9));
            case 9:
                return Integer.parseInt(documento.substring(9, 10));
        }
        return Integer.parseInt(documento.substring(9, 10));
    }

    public int getSumaRUC(String ruc, int[] coefValRuc) {
        int digito, suma = 0;
        for (int i = 0; i < coefValRuc.length; i++) {
            digito = Integer.parseInt(ruc.substring(i, i + 1)) * coefValRuc[i];
            suma = suma + digito;
        }
        return suma;
    ...
    public boolean validacionRuc(int suma, int verificador) {
        flag = false;
        int residuo = suma % 11;
        if ((residuo == 0) && (residuo == verificador)) {
            flag = true;
        } else {
            flag = (11 - residuo) == verificador;
        }
        return flag;
    }
