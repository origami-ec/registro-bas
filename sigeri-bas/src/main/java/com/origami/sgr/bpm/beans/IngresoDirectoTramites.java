...
@Named
@ViewScoped
public class IngresoDirectoTramites extends BpmManageBeanBaseRoot implements Serializable {

    private static final Logger LOG = Logger.getLogger(IngresoDirectoTramites.class.getName());
...
//            }
    ...
    public void onRowSelect(RegActo regacto) {
        try {
            if (!this.validado) {
                JsfUti.messageError(null, "Debe validar el Nro de Tramite para reingreso.", "");
                return;
            }
            acto = regacto;
            certificado = false;
...
//        pesoAcum = 0;
        ...
        if (beneficiario.getCorreo1() == null || beneficiario.getCorreo1().isEmpty()) {
            JsfUti.messageWarning(null, "Debe ingresar obligatorio correo electronico.", "");
            return false;
        }
    ...
    public void validarNroTramite() {
        RegpLiquidacion temp;
        try {
            if (liquidacion.getNumTramiteRp() != null) {
                if (liquidacion.getNumTramiteRp().compareTo(618132L) > 0
                        && liquidacion.getNumTramiteRp().compareTo(619087L) < 0) {
                    map = new HashMap();
                    map.put("numTramiteRp", liquidacion.getNumTramiteRp());
                    temp = (RegpLiquidacion) manager.findObjectByParameter(RegpLiquidacion.class, map);
                    if (temp == null) {
                        this.validado = true;
                        JsfUti.messageInfo(null, "Nro de tramite OK para reingreso.", "");
                    } else {
                        JsfUti.messageError(null, "Ya esta registrador el Nro de Tramite.", "");
                    }
                } else {
                    JsfUti.messageError(null, "El Nro de tramite NO esta dentro del rango permitido.", "");
                }
            } else {
                JsfUti.messageWarning(null, "Debe ingresar un numero de tramite.", "");
            }
        } catch (Exception e) {
            System.out.println(e);
        }
