...
@Named
@ViewScoped
public class IniciarTramiteNuevoRP extends BpmManageBeanBaseRoot implements Serializable {
    
    private static final Logger LOG = Logger.getLogger(IniciarTramiteNuevoRP.class.getName());
    ...
    protected void iniView() {
        try {
            ...
            reg.getUsuariosByRolName("certificador_ficha");
        ...
        try {
            if (acto != null) {
                if (!acto.getSolvencia()) {
                    coincidencias = itl.buscarActosIngresadosCedula(identificacion, acto);
                }
            } else {
                coincidencias = itl.buscarActosIngresadosCedula(identificacion);
            }
            System.out.println("Actos " + advertencias);
            JsfUti.update("mainForm:advGravamenes");
        } catch (Exception e) {
        ...
        try {
            if (this.validaciones()) {
                ...
                if (liquidacion == null) {
                    ...
                    String url = "/procesos/registro/iniciarTramiteNuevoRp.xhtml";
    ...
    public boolean validaciones() {
        ...
        for (RegpLiquidacionDetalles d : actosPorPagar) {
            ...
            if (d.getActo().getSolvencia()) {
                ...
                if (d.getActo().getAbreviatura() != null && !d.getActo().getAbreviatura().equals("C01")) {
