...
@Named
@ViewScoped
public class IngresoOnline extends BpmManageBeanBaseRoot implements Serializable {

    private static final Logger LOG = Logger.getLogger(IngresoOnline.class.getName());

    @Inject
    private UserSession us;

    @Inject
    private ConsultasSQLService sql;
    ...
    protected void iniView() {
        try {
            ...
            liquidaciones.addFilter("tramiteOnline", Boolean.TRUE);
...
//                    }
        ...
        try {
            ...
            if (this.comprararFechas(re.getFechaCreacion())) {
                ...
                if ((liquidacion.getTotalPagar().compareTo(BigDecimal.ZERO) == 0)
                        || liquidacion.getReingreso()) {
                    JsfUti.update("formIngresoExo");
                    JsfUti.executeJS("PF('dlgIngresoExo').show();");
                } else {
                    //sld = itl.verificarPagoSAM(liquidacion.getNumTramiteRp());
                    /*if (sld == null) {
                        JsfUti.messageWarning(null, "La solicitud NO se encuentra registrada en el SAM.", "");
                    } else {
            ...
            /*} else {
                JsfUti.messageWarning(null, "Usuario no tiene permiso asignado.", "");
            }*/
        } catch (Exception e) {
            JsfUti.messageError(null, Messages.error, "");
            LOG.log(Level.SEVERE, null, e);
    ...
    public void ingresarTramiteSinValor() {
        try {
            if (liquidacion.getId() != null) {
                ...
                JsfUti.executeJS("PF('dlgIngresoExo').hide();");
                JsfUti.update("formTramiteIngresado");
                JsfUti.executeJS("PF('dlgTramiteIngresado').show();");
                /*liquidacion = itl.asignarUsuarioSecuencias(liquidacion.getId(), cajero);
                if (liquidacion != null) {
                    this.iniciarTramiteActiviti();
                    //this.generarComprobante();
    ...
    public void continuarCobro() {
        try {
            if (liquidacion.getId() != null) {
                serv.enviarCorreoTituloCredito(liquidacion, SisVars.rutaTitulos + liquidacion.getNumTramiteRp() + ".pdf", us.getName_user());
    ...
    public void descargarVoucher(Long numTramite) {
        try {
            File file;
            map = new HashMap();
            ...
            if (solicitud != null && solicitud.getOidDocument3() != null) {
                String ruta = SisVars.rutaTemporales;
                String content = sql.getcontentTypeDoc(solicitud.getOidDocument3());
                if (content != null) {
                    switch (content) {
                        case "application/pdf":
                            ...
                            try (FileOutputStream fos = new FileOutputStream(file)) {
                                ou.streamFile(solicitud.getOidDocument3(), fos);
                                fos.close();
                            }
                            ss.instanciarParametros();
                            ss.setNombreDocumento(ruta);
                            ...
                            break;
                        case "image/jpeg":
                            ruta = ruta + numTramite.toString() + ".jpeg";
                            file = new File(ruta);
                            try (FileOutputStream fos = new FileOutputStream(file)) {
                                ou.streamFile(solicitud.getOidDocument3(), fos);
                                fos.close();
                            ...
                            ss.instanciarParametros();
                            ss.setNombreDocumento(ruta);
                            ss.setContentType("image/jpeg");
                            JsfUti.redirectNewTab(SisVars.urlbase + "DownLoadFiles");
                            break;
                        default:
                            JsfUti.messageWarning(null, "No se pudo descargar el comprobante de la transferencia.", "");
                    }
                } else {
                    JsfUti.messageWarning(null, "No se pudo descargar el comprobante de la transferencia.", "");
                }
            } else {
                JsfUti.messageWarning(null, "No se encuentra el comprobante de la transferencia.", "");
            }
        } catch (IOException e) {
            LOG.log(Level.SEVERE, null, e);
        }
    ...
    public void showDlgNotificar(RegpLiquidacion re) {
        try {
            if (cajero != null) {
                liquidacion = re;
                JsfUti.update("formBajaTramite");
                JsfUti.executeJS("PF('dlgBajaTramite').show();");
            } else {
                JsfUti.messageWarning(null, "Usuario no tiene Cajero asignado.", "");
            }
        } catch (Exception e) {
    ...
    public void cambiarEstadoLiquidacion() {
        try {
            if (!infoAdicional.isEmpty()) {
                ...
                liquidacion.setInfAdicional(infoAdicional);
                manager.merge(liquidacion);
                serv.enviarCorreoNoIngreso(liquidacion, session.getName_user());
                JsfUti.update("mainForm");
                JsfUti.executeJS("PF('dlgBajaTramite').hide();");
                JsfUti.messageInfo(null, "El trámite fue dado de baja.", "");
            } else {
                JsfUti.messageWarning(null, "Debe ingresar el motivo por el que NO se ingresa el trámite.", "");
            }
        } catch (Exception e) {
            LOG.log(Level.SEVERE, null, e);
        }
