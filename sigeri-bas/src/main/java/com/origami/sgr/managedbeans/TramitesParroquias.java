...
import com.origami.sgr.bpm.managedbeans.*;
...
@Named
@ViewScoped
public class TramitesParroquias extends BpmManageBeanBaseRoot implements Serializable {

    private static final Logger LOG = Logger.getLogger(TramitesParroquias.class.getName());
    ...
    protected void iniView() {
        try {
            ...
            liquidaciones.addFilter("estadoLiquidacion", new RegpEstadoLiquidacion(2L));
            if (!permitido) {
                liquidaciones.addFilter("userCreacion", session.getUserId());
            }
